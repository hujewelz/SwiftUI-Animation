...
struct Example5: View {
    var body: some View {
        VStack {
            FlowerView().drawingGroup() // enable Metal
        }.navigationBarTitle("Example 5").padding(20)
    }
}
...
struct FlowerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            .onAppear {
                withAnimation(Animation.easeInOut(duration: 25).repeatForever()) {
                    self.animate = true
                }
            }
        }
    ...
    var body: some View {
        ...
        return GeometryReader { proxy in
            ForEach(0..<self.petals) { i in
                ...
                ).fill(RadialGradient(gradient: Gradient(colors: [self.color.opacity(0.2), self.color]),
